...
## Git — auto sync (project policy)
- After meaningful changes in a user prompt turn: **commit and push** to `origin` (see `.cursor/rules/auto-git.mdc`).
- Never stage `.env` or secrets.
- A `stop` hook also runs `node .cursor/hooks/auto-sync.mjs` as a safety net.